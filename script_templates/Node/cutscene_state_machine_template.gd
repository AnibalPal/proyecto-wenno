# class_name EntityNameHereCutsceneStateMachine
extends CutsceneStateMachine

# Put state names here, must be consistent with the node names
const IDLE := "Idle"
const MOVE := "Move"
...
func execute_cutscene_step(action: Enums.CutsceneCommonActions, step_data = {}) -> void:
	# Transform available cutscene actions for the entity
	pass
	#match action:
